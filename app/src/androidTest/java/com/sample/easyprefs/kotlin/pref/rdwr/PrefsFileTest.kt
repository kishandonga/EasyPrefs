...
class PrefsFileTest {
    ...
    fun test1_commitOp() {
        ...
        val o1 = Prefs.read(Const.PREF_FILE).content(Const.STRING_KEY, "")
        assertEquals(p1, o1)

        val o2 = Prefs.read(Const.PREF_FILE).content(Const.INT_KEY, Int.MIN_VALUE)
        assertEquals(p2, o2)

        val o3 = Prefs.read(Const.PREF_FILE).content(Const.FLOAT_KEY, Float.MIN_VALUE)
        assertEquals(p3, o3)

        val o4 = Prefs.read(Const.PREF_FILE).content(Const.LONG_KEY, Long.MIN_VALUE)
        assertEquals(p4, o4)

        val o5 = Prefs.read(Const.PREF_FILE).content(
            Const.DOUBLE_KEY,
            Double.MIN_VALUE
        )
        assertEquals(p5.toString(), o5.toString())

        val o6 = Prefs.read(Const.PREF_FILE).content(Const.BOOLEAN_KEY, true)
        assertEquals(p6, o6)

        val o7 = Prefs.read(Const.PREF_FILE).content(Const.SET_KEY, setOf())
        assertEquals(p7, o7)
    }
    ...
    fun test2_applyOp() {
        ...
        val o1 = Prefs.read(Const.PREF_FILE).content(Const.STRING_KEY_APPLY, "")
        assertEquals(p1, o1)

        val o2 = Prefs.read(Const.PREF_FILE).content(
            Const.INT_KEY_APPLY,
            Int.MIN_VALUE
        )
        assertEquals(p2, o2)

        val o3 = Prefs.read(Const.PREF_FILE).content(
            Const.FLOAT_KEY_APPLY,
            Float.MIN_VALUE
        )
        assertEquals(p3, o3)

        val o4 = Prefs.read(Const.PREF_FILE).content(
            Const.LONG_KEY_APPLY,
            Long.MIN_VALUE
        )
        assertEquals(p4, o4)

        val o5 = Prefs.read(Const.PREF_FILE).content(
            Const.DOUBLE_KEY_APPLY,
            Double.MIN_VALUE
        )
        assertEquals(p5.toString(), o5.toString())

        val o6 = Prefs.read(Const.PREF_FILE).content(Const.BOOLEAN_KEY_APPLY, true)
        assertEquals(p6, o6)

        val o7 = Prefs.read(Const.PREF_FILE).content(
            Const.SET_KEY_APPLY,
            setOf()
        )
        assertEquals(p7, o7)
