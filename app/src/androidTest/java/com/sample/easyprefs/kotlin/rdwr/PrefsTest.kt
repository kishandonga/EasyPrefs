...
class PrefsTest {
    ...
    fun test2_ReadCommitOp() {
        ...
        val o1 = Prefs.read().content(Const.SAMPLE_STRING_KEY, "")
        assertEquals(p1, o1)

        val o2 = Prefs.read().content(Const.SAMPLE_INT_KEY, Int.MIN_VALUE)
        assertEquals(p2, o2)

        val o3 = Prefs.read().content(Const.SAMPLE_FLOAT_KEY, Float.MIN_VALUE)
        assertEquals(p3, o3)

        val o4 = Prefs.read().content(Const.SAMPLE_LONG_KEY, Long.MIN_VALUE)
        assertEquals(p4, o4)

        val o5 = Prefs.read().content(Const.SAMPLE_DOUBLE_KEY, Double.MIN_VALUE)
        assertEquals(p5.toString(), o5.toString())

        val o6 = Prefs.read().content(Const.SAMPLE_BOOLEAN_KEY, true)
        assertEquals(p6, o6)

        val o7 = Prefs.read().content(Const.SAMPLE_STRING_SET_KEY, setOf())
        assertEquals(p7, o7)
    }
    ...
    fun test3_ApplyOp() {
        ...
        val o2 = Prefs.read().content(Const.SAMPLE_INT_KEY_APPLY, Int.MIN_VALUE)
        assertEquals(p2, o2)

        val o3 = Prefs.read().content(Const.SAMPLE_FLOAT_KEY_APPLY, Float.MIN_VALUE)
        assertEquals(p3, o3)

        val o4 = Prefs.read().content(Const.SAMPLE_LONG_KEY_APPLY, Long.MIN_VALUE)
        assertEquals(p4, o4)

        val o5 = Prefs.read().content(Const.SAMPLE_DOUBLE_KEY_APPLY, Double.MIN_VALUE)
        assertEquals(p5.toString(), o5.toString())

        val o6 = Prefs.read().content(Const.SAMPLE_BOOLEAN_KEY_APPLY, true)
        assertEquals(p6, o6)

        val o7 = Prefs.read().content(Const.SAMPLE_STRING_SET_KEY_APPLY, setOf())
        assertEquals(p7, o7)
    }
}